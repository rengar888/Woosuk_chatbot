...
# =========================================================================
# 💡 [요청 사항 반영] 함수를 사용하지 않고 데이터를 즉시 로드하는 로직
# =========================================================================
ACADEMIC_CHUNKS = []
FILE_PATH = 'academics.txt'
print(f"데이터 파일 {FILE_PATH} 로드 시도...")
try:
    with open(FILE_PATH, 'r', encoding='utf-8') as f:
        content = f.read()
        # 줄 바꿈을 기준으로 문장을 청크로 사용
        for line in content.split('\n'):
            line = line.strip()
            # 빈 줄이나 제목 줄은 제외
            if line and not line.startswith('[제목:'):
                ACADEMIC_CHUNKS.append(line)
    print(f"✅ {FILE_PATH}에서 {len(ACADEMIC_CHUNKS)}개의 청크 로드 완료.")
except FileNotFoundError:
    print(f"❌ 데이터 파일 {FILE_PATH}를 찾을 수 없습니다. 기본 청크만 사용합니다.")
except Exception as e:
    print(f"❌ 데이터 로드 중 오류 발생: {e}")

# 최종적으로 Faiss 인덱스에 들어갈 전체 청크
DOCUMENT_CHUNKS = BASE_CHUNKS + ACADEMIC_CHUNKS
# =========================================================================
...
def retrieve_relevant_context(query, index, model, chunks, k=4):
...
def initialize_search_engine():
    ...
    # 이미 전역 변수 DOCUMENT_CHUNKS가 구성되었는지 확인
    if not DOCUMENT_CHUNKS:
        print("❌ 로드할 데이터가 없어 엔진 초기화를 중단합니다.")
        return False

    try:
        # 1. 검색 모델 (Sentence Transformer) 로드 및 Faiss 인덱스 구축
        print(f"1. 검색 엔진 초기화 중... (총 {len(DOCUMENT_CHUNKS)}개 청크 사용)")
